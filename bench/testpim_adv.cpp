...
int main()
{
	...
	*heap_alloc_3 = 2;
	...
	for(uint32_t itr = 0; itr < 16; ++itr)
	{
		heap_alloc_1[itr] = itr + 1;
		heap_alloc_2[itr] = itr + 2;
	}
	...
	*pim_addr = 1;
	...
	{
		if(heap_alloc_3[itr] != itr + 1 + itr + 2)
		{
			fprintf(stdout, "itr: %d is not correct.\n", itr);
			++error;
		}
	}
	...
	fprintf(stdout, "Trying PIM subtraction...\n");
	*halloc_4 = static_cast<uint64_t>(pim::MCC_SUB);
	*pim_addr = 1;

	error = 0;
	for(size_t itr = 0; itr < 16; ++itr)
	{
		if(heap_alloc_3[itr] != -1)
		{
			fprintf(stdout, "itr: %d is not correct.\n", itr);
			++error;
		}
	...
	if(!error)
	{
		fprintf(stdout, "Vectorized subtraction test passed.\n");
		fprintf(stdout, "heap_alloc_3[0] = %d\n", heap_alloc_3[0]);
	}

	fprintf(stdout, "Trying PIM multiplication...\n");
	*halloc_4 = static_cast<uint64_t>(pim::MCC_MUL);
	*pim_addr = 1;
	...
	if(!error)
	{
		fprintf(stdout, "Vectorized multiplication test passed.\n");
		fprintf(stdout, "heap_alloc_3[0] = %d\n", heap_alloc_3[0]);
	}
